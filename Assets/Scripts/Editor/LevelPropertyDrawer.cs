    ...
    public static void OnGUI(Rect position, SerializedObject serializedObject, GUIContent label = null) {
      ...
      for (var i = 0; i < subSceneProp.arraySize; ++i) {
        ...
        EditorGUI.PropertyField(
          subSceneArrayElementPosition,
          subSceneProp.GetArrayElementAtIndex(i),
          new GUIContent(i.ToString()));
        if (GUI.Button(xButtonPosition, new GUIContent("×"))) subSceneProp.DeleteArrayElementAtIndex(i);
      }
